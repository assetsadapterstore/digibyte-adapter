...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	...
	digibyte_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := digibyte_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := digibyte_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
...
func (decoder *addressDecoder) ScriptPubKeyToBech32Address(scriptPubKey []byte) (string, error) {
	...
	cfg := digibyte_addrdec.DGB_mainnetAddressBech32V0
	if decoder.wm.Config.IsTestNet {
		cfg = digibyte_addrdec.DGB_testnetAddressBech32V0
	}
